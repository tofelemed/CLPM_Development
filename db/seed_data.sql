-- Seed data for CLPM database

-- Insert sample loop configurations
...
-- Insert sample raw data for the last 24 hours
...
SELECT 
    ...
    l.id as loop_id,
    -- Generate realistic process values
    50 + 10 * sin(EXTRACT(EPOCH FROM generate_series(NOW() - INTERVAL '24 hours', NOW(), INTERVAL '1 minute')) / 3600) + (random() - 0.5) * 5 as pv,
    45 + 5 * sin(EXTRACT(EPOCH FROM generate_series(NOW() - INTERVAL '24 hours', NOW(), INTERVAL '1 minute')) / 1800) + (random() - 0.5) * 3 as op,
    50 as sp,
    'AUTO' as mode,
    45 + 5 * sin(EXTRACT(EPOCH FROM generate_series(NOW() - INTERVAL '24 hours', NOW(), INTERVAL '1 minute')) / 1800) + (random() - 0.5) * 3 as valve_position,
    192 as quality_code
FROM loops l
WHERE l.name = 'Temperature Control Loop'
ON CONFLICT DO NOTHING;

-- Insert sample KPI results for the last 7 days
...
SELECT 
    ...
    0.85 + (random() - 0.5) * 0.1 as service_factor,
    0.82 + (random() - 0.5) * 0.08 as effective_sf,
    0.15 + (random() - 0.5) * 0.1 as sat_percent,
    45.5 + (random() - 0.5) * 10 as output_travel,
    0.78 + (random() - 0.5) * 0.15 as pi,
    0.82 + (random() - 0.5) * 0.12 as rpi,
    0.25 + (random() - 0.5) * 0.2 as osc_index,
    0.18 + (random() - 0.5) * 0.15 as stiction
FROM loops l
WHERE l.name = 'Temperature Control Loop'
ON CONFLICT DO NOTHING;

-- Insert sample diagnostic results
...
SELECT 
    ...
    0.15 + (random() - 0.5) * 0.1 as stiction_S,
    0.12 + (random() - 0.5) * 0.08 as stiction_J,
    18.5 + (random() - 0.5) * 10 as stiction_pct,
    120.5 + (random() - 0.5) * 60 as osc_period,
    CASE 
        WHEN random() > 0.7 THEN 'STICTION'
        WHEN random() > 0.5 THEN 'OSCILLATION'
        ELSE 'NORMAL'
    END as classification,
    '{"confidence": 0.85, "recommendation": "Monitor valve performance"}'::jsonb as details
FROM loops l
WHERE l.name = 'Temperature Control Loop'
ON CONFLICT DO NOTHING;

-- Insert aggregated data (1-minute intervals)
...
-- ============================================================================
-- INSERT 10 MORE LOOPS WITH MIXED STATUSES
-- ============================================================================
